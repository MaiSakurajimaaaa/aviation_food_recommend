...
const getOrderListData = async (status: number) => {
  ...
  orderData.value = responseData.records;
  counts.value = responseData.total;
  if (dialogOrderStatus.value === 2 && status === 2 && isAutoNext.value
    && !isTableOperateBtn.value && responseData.records.length > 1) {
    const firstRow = responseData.records[0];
