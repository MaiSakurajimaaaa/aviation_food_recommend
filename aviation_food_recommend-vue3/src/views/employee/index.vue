...
const update_btn = (row: EmployeeRow) => {
  router.push({
...
// 修改员工状态
const change_btn = async (row: EmployeeRow) => {
...
// 删除员工
const delete_btn = (row: EmployeeRow) => {
...
const canEditRow = (row: EmployeeRow) => {
  ...
  return isSuperAdmin(currentAccount) || currentAccount === row.account
}

const canManageAll = () => {
  return isSuperAdmin(userInfoStore.userInfo?.account)
}
</script>
...
<template>
  <el-card>
    <div class="horizontal">
      ...
      <el-button v-if="canManageAll()" size="large" class="btn" type="primary" @click="router.push('/employee/add')">
    ...
    <el-table :data="employeeList" stripe>
      ...
      <el-table-column label="操作" width="200px" align="center">
        ...
        <template #default="scope">
          ...
          <el-button @click="update_btn(scope.row)" type="primary" :disabled="!canEditRow(scope.row)">修改
          </el-button>
